    ...
    <base-material-card>
      <template v-slot:heading>
        <div class="text-h3 font-weight-light">
          {{ modeEdit ? "Editar" : "Registrar" }} Egreso
        </div>
      </template>
      <v-form
        id="formEgreso"
      ...
      >
        ...
        >
          <v-text-field
            v-model.number="egreso.cantidad"
            label="Cantidad"
            name="cantidad"
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.egreso.userId = this.userGetter.id
    },
    methods: {
      close () {
        this.$emit('close')
